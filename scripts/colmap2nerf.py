...
colmap_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'COLMAP\\') 
...
def run_ffmpeg(args):
    ...
    do_system(f"{ffmpeg_path}ffmpeg.exe -i {video} -qscale:v 1 -qmin 1 -vf \"fps={fps}{time_slice_value}\" {images}/%04d.jpg")
...
def run_colmap(args):
    ...
    if os.path.exists(db):
        os.remove(db)
    do_system(f"{colmap_path}colmap feature_extractor --ImageReader.camera_model OPENCV --SiftExtraction.estimate_affine_shape=true --SiftExtraction.domain_size_pooling=true --ImageReader.single_camera 1 --database_path {db} --image_path {images}")
    do_system(f"{colmap_path}colmap {args.colmap_matcher}_matcher --SiftMatching.guided_matching=true --database_path {db}")
    try:
        shutil.rmtree(sparse)
    except:
        pass
    do_system(f"mkdir {sparse}")
    do_system(f"{colmap_path}colmap mapper --database_path {db} --image_path {images} --output_path {sparse}")
    do_system(f"{colmap_path}colmap bundle_adjuster --input_path {sparse}/0 --output_path {sparse}/0 --BundleAdjustment.refine_principal_point 1")
    try:
        shutil.rmtree(text)
    except:
        pass
    do_system(f"mkdir {text}")
    do_system(f"{colmap_path}colmap model_converter --input_path {sparse}/0 --output_path {text} --output_type TXT")
